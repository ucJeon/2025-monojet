...
make_table.py
-------------
AsymptoticLimits ROOT 파일을 읽어 systematic study 결과 테이블을 출력합니다.

실행 방법 (CMSSW 환경 source 후):
  python3 make_table.py                     # 전체 모드 테이블
  python3 make_table.py --sys stats         # 특정 모드만
  python3 make_table.py --sys sys1
  python3 make_table.py --lumi 300          # 특정 lumi만
  python3 make_table.py --out result.txt    # 파일로 저장
  python3 make_table.py --fmt latex         # LaTeX 형식
  python3 make_table.py --save-result       # z_asymp_result_{mode}.txt 저장
  python3 make_table.py --sys stats --save-result  # 특정 모드만 저장
"""
...
def get_root_path(base_dir: str, lumi: int, mx_tag: str, mass: int, mode: str) -> str:
    ...
    root_name = (f"higgsCombine.Lumi{lumi}.{mx_tag}.{mode}"
                 f".AsymptoticLimits.mH{mass}.root")
